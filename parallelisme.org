...
*** Le programme "fil rouge"
  ...
  - Chaque thread attends 5 seconde avant de terminer

  Pour peaufiner la visualisation, le petit programme suivant, lit son
  entrée standart et affiche chaque ligne lue avec la date courante
  depuis son démarrage (en seconde)

#+INCLUDE: "./tempusfugit.pl" src perl

  Du coup, les programes forcent l'affichage immédiat (flush) pour
  éviter tout délai dans le pipe séparant les programmes de celui
  marquant les dates d'apparitions des messages.
